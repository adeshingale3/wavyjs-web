...
import { buttonsData } from '../JSONinfo/Buttons';
 ...
 overflow-auto scrollbar-hidden">
      {buttonsData.map((btn, index) => (
        <div key={index} className='flex flex-col mb-16 sm:mb-20 md:mb-28 lg:mb-32'>
          {/* Heading */}
          <div>
            <h1 className="text-3xl sm:text-4xl md:text-6xl text-white font-bold">{btn.title}</h1>
          <h3 className="text-sm sm:text-base md:text-lg text-white/40 font-semibold">{btn.description}</h3>
          ...
          <div className="w-full mt-6 sm:mt-10 flex flex-col">
            ...
            <div className="w-[95%] border border-white/20 bg-white/5 text-white/90 rounded-lg px-3 py-2 sm:px-5 sm:py-4 mt-1 sm:mt-3 font-mono text-xs sm:text-sm">{btn.installCommand}
            </div>
