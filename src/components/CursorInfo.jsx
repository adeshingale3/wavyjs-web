...
import { cursorData } from '../JSONinfo/Cursor';
...
const CursorInfo = () => {
  ...
  return (
    <div className="h-screen w-screen p-4 sm:p-6 md:p-10 border border-white/30 rounded-xl flex flex-col gap-10 sm:gap-12 md:gap-16 lg:gap-20
 overflow-auto scrollbar-hidden">
      {cursorData.map((cursor, index) => (
        <div key={index} className='flex flex-col mb-16 sm:mb-20 md:mb-28 lg:mb-32'>
          {/* Heading */}
          <div>
            <h1 className="text-3xl sm:text-4xl md:text-6xl text-white font-bold">{cursor.title}</h1>
          <h3 className="text-sm sm:text-base md:text-lg text-white/40 font-semibold">{cursor.description}</h3>
          ...
          <div className="w-full mt-6 sm:mt-10 flex flex-col">
            ...
            <div className="w-[95%] border border-white/20 bg-white/5 text-white/90 rounded-lg px-3 py-2 sm:px-5 sm:py-4 mt-1 sm:mt-3 font-mono text-xs sm:text-sm">{cursor.installCommand}
            </div>
